...
if self.ui.radioButtonSpatial.isChecked() == True and Lafs > Lafg:
...
if self.ui.radioButtonPerimetral.isChecked()  == True and Lafs > Lafg:
    Wtotal =Wtotal * Lafs / Lafg
if tw1 != 0:
...
for (Ele, EleForceD, EleForceDL, EleForceDLE) in zip(Elements, ElemnsForceD, ElemnsForceDL, ElemnsForceDLE):
    ...
    if Ele.EleTag in ListEleTagBeams:
        ...
        VI3 = 0.9 * VID - 1.0 * VIE
        VI4 = abs(-(Mn_P1 + Mn_N2) / Ele.LEle + ((1.2 + 0.2*Sds)*WDL + 1.0*WLL)/FN * Ele.LEle / 2.)
        VI5 = (Mn_N1 + Mn_P2) / Ele.LEle + ((1.2 + 0.2*Sds)*WDL + 1.0*WLL)/FN * Ele.LEle / 2.
        VI6 = 1.2 * VID + 1.0 * VIL - 2 * VIE
        VI7 = 0.9 * VID - 2 * VIE
        ...
        VE3 = 0.9 * VED + 1.0 * VEE
        VE4 = (Mn_P1 + Mn_N2) / Ele.LEle + ((1.2 + 0.2*Sds)*WDL + 1.0*WLL)/FN * Ele.LEle / 2.
        VE5 = abs(-(Mn_N1 + Mn_P2) / Ele.LEle + ((1.2 + 0.2*Sds)*WDL + 1.0*WLL)/FN * Ele.LEle / 2.)
        VE6 = 1.2 * VED + 1.0 * VEL + 2 * VEE
        VE7 = 0.9 * VED + 2 * VEE
        ...
        nb1 = max(nbt1, nbb1)
        nb2 = max(nbt2, nbb2)
        nst1, sst1, sem1 = AvBeam(VU1, db_t1, dt1, Ele.EleTag, fys, dst, Ast, b)
        nst2, sst2, sem2 = AvBeam(VU2, db_t2, dt2, Ele.EleTag, fys, dst, Ast, b)
...
for (Ele, EleForceD, EleForceDL, EleForceDLE) in zip(Elements, ElemnsForceD, ElemnsForceDL, ElemnsForceDLE):
    ...
    if Ele.EleTag in ListEleTagCols:
        ...
        nbH, nbB, db, As, fiPn, fiMn, Mn_i, Mpr_i, d, dist, ro, Mu_i, ColBeamStr = AsColumn(b, h, Ele.EleTag, cover, dst,
                                                                                            fy, Beta1C, Pu_v, Mu_v,
        ...
        VI2 = 1.2 * VID + 1.0 * VIL + 1.0 * VIE
        VI3 = 0.9 * VID + 1.0 * VIE
        VI4 = (2*Mn_max) / Ele.LEle 
        VI6 = 1.2 * VID + 1.0 * VIL + 2 * VIE
        VI7 = 0.9 * VID + 2 * VIE

        VUa = np.max(np.abs([VI1, VI2, VI3]))
        VUb = VI4
        VUc = max(VI6, VI7)

        Vu = max(VUa, min(VUb, VUc))  # Negative shear node final design
