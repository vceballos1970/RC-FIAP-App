    ...
    for nde in de_v:
        ...
        for neB in neB_v:
            for neH in neH_v:
                Ash_H = neH * Ast
                Ash_B = neB * Ast
                se_1 = min(Ash_H / bc1 / (0.15 * (Ag / (bc1 * bc2) - 1) * fcC / fys),
                           Ash_B / bc2 / (0.15 * (Ag / (bc1 * bc2) - 1) * fcC / fys),
                           Ash_H / bc1 / (0.06 * fcC / fys),
                           Ash_B / bc2 / (0.06 * fcC / fys))
                if se_1 <= 10*cm:
                    se_1 = 10*cm
                    de = 3 / 8. * inch
                    neH = ceil(bc1/(20*cm))
                    ...
                    Ash_B = neB * Ast
                Vs = (Vu - fiv * Vc) / fiv
                se_1 = min(se_1, h/2, b/2, 8 * db, 24 * de)
...
if self.ui.radioButtonSpatial.isChecked() and Lafs > Lafg:
    PColSlabD = (Lafs - Lafg) * DL + Pvig / FN * Lafs  # esto lo debo arreglar falta una dimension en la primera parte
...
for (Ele, EleForceD, EleForceDL, EleForceDLE) in zip(Elements, ElemnsForceD, ElemnsForceDL, ElemnsForceDLE):
    ...
    if Ele.EleTag in ListEleTagBeams:
        ...
        VU1 = max(VU1a, min(VU1b, VU1c))  # Negative shear node final design
...
for (Ele, EleForceD, EleForceDL, EleForceDLE) in zip(Elements, ElemnsForceD, ElemnsForceDL, ElemnsForceDLE):
    ...
    if Ele.EleTag in ListEleTagCols:
        ...
        VU1b = VI6
        VU1c = max(VI7, VI8)
        Vu = max(VU1a, min(VU1b, VU1c))  # Negative shear node final design
