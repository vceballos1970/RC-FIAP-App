...
global Loc_span, Loc_heigth, ListNodes, Elements, DataBeamDesign, DataColDesign, Wtotal, num_elems, ListNodesDrift, \
    cIndex, ListNodesBasal, T1m, IM, Sa_max, RDR_max, SDR_max, nrecs, DataWallDesign, net, nnt, ListNodesBasal1, nnt1
...
from models import AcceptanceCriteria, PlasticHingeLength, WallDesign, ColDesign, BeamDesign, ZeroLengthElement, BeamElasticElement, RegistroWalls
...
class MyForm(QMainWindow):
    ...
    def CreateNLM(self):
        ...
        def fiber_beam(Ast, Asb, pos, FNB):
            op.section('Fiber', Ele.EleTag * 10 + pos)
            ...
            if NIF != 0 and FNB > 0:
                op.section('Fiber', Ele.EleTag * 10 + 5 + pos)
                op.patch('rect', Ele.EleTag * 20 + 2 + pos, NFBeams, 1, -y2 + dp, -z2BF + dp1BF, y2 - dp, z2BF - dp1BF)
                op.patch('rect', Ele.EleTag * 20 + pos, NFBeams, 1, -y2 + dp, z2BF - dp1BF, y2 - dp, z2BF)
                op.patch('rect', Ele.EleTag * 20 + pos, NFBeams, 1, -y2 + dp, -z2BF, y2 - dp, -z2BF + dp1BF)
                op.patch('rect', Ele.EleTag * 20 + pos, 2, 1, -y2, -z2BF, -y2 + dp, z2BF)
                op.patch('rect', Ele.EleTag * 20 + pos, 2, 1, y2 - dp, -z2BF, y2, z2BF)
                # print(Ele.EleTag * 20 + 4 + pos, 1, Ast/cm**2, y2 - dp, z2BF - dp, y2 - dp, -z2BF + dp)
                op.layer('straight', Ele.EleTag * 20 + 4 + pos, 1, Ast * FNB, y2 - dp, z2BF - dp1BF, y2 - dp, -z2BF + dp1BF)
                # print(Ele.EleTag * 20 + 4 + pos, 1, Asb/cm**2, -y2 + dp, z2BF - dp, -y2 + dp, -z2BF + dp)
                op.layer('straight', Ele.EleTag * 20 + 4 + pos, 1, Asb * FNB, -y2 + dp, z2BF - dp1BF, -y2 + dp, -z2BF + dp1BF)

        op.wipe()  # The models is restarted in opensees
        op.model('Basic', '-ndm', 2, '-ndf', 3)
        NIF = np.count_nonzero(infill_m)
        FN = float(self.ui.frames_numbers.text())
        if self.ui.radioButtonPerimetral.isChecked() == True:
            FN = 1
        FNB = FN - 1
        WDLS = WDLS / FN
        WDL = WDL / FN
        ...
        for node in ListNodes:
            ...
            if NIF != 0 and FNB > 0:  op.node(int(node[0] + nnt1), node[1], node[2])
            if node[2] == 0.:
                op.fix(int(node[0]), 1, 1, 1)
                if NIF != 0 and FNB > 0:
                    op.fix(int(node[0] + nnt1), 1, 1, 1)
        ...
        for node in ListNodes:
            if node[2] == 0.:
                ...
                if NIF != 0 and FNB > 0:
                    op.node(int(node[0] + nnt1) + nntot * 2, node[1], node[2])
            if node[1] == 0. and node[2] != Loc_heigth[-1] and node[2] != 0.:
                ...
                if NIF != 0 and FNB > 0:
                    op.node(int(node[0] + nnt1) + nntot * 2, node[1], node[2])
                    op.node(int(node[0] + nnt1) + nntot * 3, node[1], node[2])
        ...
        if self.ui.radioButtonJS1.isChecked() == True:
        ...
        if self.ui.radioButtonYesLC.isChecked():
            ...
            for Nod_ini in range(int(ListNodesLC[0, 0]), int(ListNodesLC[-1, 0])):
                ...
                op.element('elasticBeamColumn', EleTag * 20, Nod_ini, Nod_end, 1e3, EcC, Iner_rat * IzCol, 6)
                ...
                if ListNodes[Nod_ini, 1] == Loc_span[-1] and ListNodes[Nod_ini, 2] != 0.:
                    for indLC in range(ListNodesLC[:, 0].size):
                        if ListNodesLC[indLC, 2] == ListNodes[Nod_ini, 2]:
                            ...
                            op.element("corotTruss", EleTag * 20, Nod_ini, Nod_end, 1e3, int(1e4))
                            # print("corotTruss", EleTag, Nod_ini, Nod_end, 1e3, int(1e4))
                            EleTag += 1
            net = EleTag - 1
        ...
        for (Ele, DC, IFE, lwIF) in zip(EleCol, DataColDesign, infill_m, infill_lw):
            ...
            y1 = DC.h / 2.0
            z1 = DC.b / 2.0 * FN
            op.section('Fiber', Ele.EleTag * 10)
            ...
            op.beamIntegration('HingeRadau', Ele.EleTag * 2, Ele.EleTag * 10, phl, Ele.EleTag * 10, phl, Ele.EleTag * 10)
            op.element('forceBeamColumn', Ele.EleTag * 20, Nod_ini, Nod_end, Ele.ElegTr, Ele.EleTag * 2, '-mass', MassDens)
            if NIF != 0 and FNB > 0:
                dp1BF = dp * FNB
                z1BF = DC.b / 2.0 * FNB
                op.section('Fiber', Ele.EleTag * 10 + 5)
                op.patch('rect', Ele.EleTag * 20 + 2, NFCols, 1, -y1 + dp, -z1BF + dp1BF, y1 - dp, z1BF - dp1BF)
                op.patch('rect', Ele.EleTag * 20, NFCols, 1, -y1 + dp, z1BF - dp1BF, y1 - dp, z1BF)
                op.patch('rect', Ele.EleTag * 20, NFCols, 1, -y1 + dp, -z1BF, y1 - dp, -z1BF + dp1BF)
                op.patch('rect', Ele.EleTag * 20, 2, 1, -y1, -z1BF, -y1 + dp, z1BF)
                op.patch('rect', Ele.EleTag * 20, 2, 1, y1 - dp, -z1BF, y1, z1BF)
                for dist, As in zip(DC.dist, DC.As):
                ...
                op.element('forceBeamColumn', Ele.EleTag * 20 + 10, Nod_ini, Nod_end, Ele.ElegTr, Ele.EleTag * 2 + 1, '-mass', MassDens)
            EleTag = int(Ele.EleTag * 20 + 2)
            ...
            if NIF != 0 and FNB > 0:
                op.element('zeroLength', EleTag + 10, Nod_ini + nnt1, Nod_end + nnt1, '-mat', MatTagY, MatTagX, MatTagZ,
                           '-dir', 1, 2, 3)
            EleTag = int(Ele.EleTag * 20 + 1)
            Nod_ini = Ele.Nod_ini
        ...
        for (Ele, DW) in zip(EleWall, DataWallDesign):
            ...
            if self.ui.radioButtonFBC.isChecked():
                ...
                thickMVLEM, widthsMVLEM, rhoMVLEM, ConcMVLEM, SteelMVLEM, ShearMVLEM = thickMVLEM.tolist(),\
                    ...
                    ShearMVLEM.tolist()
                op.element('MVLEM', Ele.EleTag * 20, MassDens, *eleNodes, NFMVLEM, 0.4, '-thick', *thickMVLEM, '-width',
                           *widthsMVLEM, '-rho', *rhoMVLEM, '-matConcrete', *ConcMVLEM, '-matSteel', *SteelMVLEM,
                           '-matShear', *ShearMVLEM)
                # print('MVLEM', Ele.EleTag * 20, MassDens, *eleNodes, NFMVLEM, 0.4, '-thick', *thickMVLEM, '-width',
                #            *widthsMVLEM, '-rho', *rhoMVLEM, '-matConcrete', *ConcMVLEM, '-matSteel', *SteelMVLEM,
                #            '-matShear', *ShearMVLEM)
        ...
        for (Ele, DB) in zip(EleBeam, DataBeamDesign):
            ...
            # print('dp1 =', dp)
            pos = 0
            fiber_beam(DB.Ast1 * FN, DB.Asb1 * FN, pos, FNB)
            ...
            # print('dp2 =', dp)
            pos = 1
            fiber_beam(DB.Ast2 * FN, DB.Asb2 * FN, pos, FNB)
            LColT = Ele.LEle + Ele.RZi + Ele.RZe
            MassDens = ((Ele.AEle * GConc / g) * FN + WDLS * FN / g) * (LColT / Ele.LEle)
            Nod_ini = Ele.Nod_ini  # + nntot * 4
            Nod_end = Ele.Nod_end  #+ nntot * 5
            op.beamIntegration('HingeRadau', Ele.EleTag * 2, Ele.EleTag * 10, phl1, Ele.EleTag * 10 + 1, phl2, Ele.EleTag * 10)
            op.element('forceBeamColumn', Ele.EleTag * 20, Nod_ini, Nod_end, Ele.ElegTr, Ele.EleTag * 2, '-mass', MassDens)
            if NIF != 0 and FNB > 0:
                MassDens = ((Ele.AEle * GConc / g) * FNB + WDLS * FNB / g)  * (LColT / Ele.LEle)
                Nod_ini = Ele.Nod_ini  # + nntot * 4
                Nod_end = Ele.Nod_end  # + nntot * 5
                op.beamIntegration('HingeRadau', Ele.EleTag * 2 + 1, Ele.EleTag * 10 + 5, phl1, Ele.EleTag * 10 + 5, phl2, Ele.EleTag * 10 + 5)
                op.element('forceBeamColumn', Ele.EleTag * 20 + 10, Nod_ini + nnt1, Nod_end + nnt1, Ele.ElegTr, Ele.EleTag * 2 + 1, '-mass', MassDens)
        ...
        for Ele in ZeroLengths:
            ...
            op.element('zeroLength', Ele.EleTag * 20, Ele.Nod_ini, Ele.Nod_end, '-mat', Ele.EleTag * 20, '-dir', 1)
        if NIF != 0 and FNB > 0:
            for Ele in EleCol:
        ...
        for Ele in EleCol:
            ...
            PColSlabD1 = (PColSlabD + HCole * WDL) * FN
            PColSlabL1 = PColSlabL * FN
            op.eleLoad('-ele', Ele.EleTag * 20, '-type', '-beamUniform', 0, -PCol * FN)
            op.eleLoad('-ele', Ele.EleTag * 20, '-type', '-beamPoint', 0, 1, -PColSlabD1 - 0.25 * PColSlabL1)
            if NIF != 0 and FNB > 0:
                PColSlabD1 = (PColSlabD + HCole * WDL) * FNB
                PColSlabL1 = PColSlabL * FNB
                op.eleLoad('-ele', Ele.EleTag * 20 + 10, '-type', '-beamUniform', 0, -PCol * FNB)
                op.eleLoad('-ele', Ele.EleTag * 20 + 10, '-type', '-beamPoint', 0, 1, -PColSlabD1 - 0.25 * PColSlabL1)
        for Ele in EleWall:
            if self.ui.radioButtonFBC.isChecked():
                op.eleLoad('-ele', Ele.EleTag * 20, '-type', '-beamUniform', 0, -Ele.AEle * GConc)
                if np.any(ListNodesW1):
                    op.eleLoad('-ele', Ele.EleTag * 20, '-type', '-beamPoint', 0, 1, -PWall1SlabD - 0.25 * PWall1SlabL)
                if np.any(ListNodesW2):
                    op.eleLoad('-ele', Ele.EleTag * 20, '-type', '-beamPoint', 0, 1, -PWall2SlabD - 0.25 * PWall2SlabL)
            elif self.ui.radioButtonMVLEM.isChecked():
        ...
        for Ele in EleBeam:
            op.eleLoad('-ele', Ele.EleTag * 20, '-type', '-beamUniform',
                       (-Ele.AEle * GConc) * FN - WDL * FN - 0.25 * WLL * FN)
            if NIF != 0 and FNB > 0:
                op.eleLoad('-ele', Ele.EleTag * 20 + 10, '-type', '-beamUniform',
                           (-Ele.AEle * GConc) * FNB - WDL * FNB - 0.25 * WLL * FNB)
        if self.ui.radioButtonYesLC.isChecked():
            PD_LC = (WDLS * FN - WDL * FN) * Loc_span[-1]
            PL_LC = (WDLS * FN - WDL * FN) / WDL * WLL * Loc_span[-1] * FN**2
        ...
        for (Ele, ind) in zip(EleBeam, range(num_beams)):
            PDG = op.eleResponse(Ele.EleTag * 20, 'plasticDeformation')
    ...
    def Pushover(self):
        ...
        def singlePush1(dref, mu, ctrlNode, dispDir, nSteps):
            ...
            while step <= nSteps and ok == 0 and loadf > 0:
                self.ui.progressBarPushover.setValue(int(100 * step / nSteps))
                ElemsForceS1, ElemsDeforS1, ElemsForceS6, ElemsDeforS6 = [], [], [], []
                ok = op.analyze(1)
                loadf = op.getTime()
                ...
                if NIF != 0 and FNB > 0:
                    VBasalBare = 0
                    for node in ListNodesBasal1:
                        VBasalBare = VBasalBare + op.nodeReaction(int(node[0] + nnt1), 1)
                    VBasal_vBare = np.append(VBasal_vBare, VBasalBare)
            ...
            return maxDriftPiso, maxDriftTecho, maxDriftTecho2, VBasal_v, MaxDriftStory_v, DriftTecho_v, \
                DriftStory_m, PD_Beams, PD_Cols, PD_Walls, Vu_Vn_Cols, ratio_infill, VBasal_vBare
        ...
        if self.ui.radioButtonFast.isChecked():
            ...
            maxDriftPiso, maxDriftTecho, maxDriftTecho2, VBasal_v, MaxDriftStory_v, DriftTecho_v, DriftStory_m, \
                PD_Beams, PD_Cols, PD_Walls, Vu_Vn_Cols, ratio_infill, VBasal_vBare = singlePush1(dref, mu, IDctrlNode,
                                                                                                  IDctrlDOF, nSteps)
        ...
        if self.ui.radioButtonForced.isChecked():
            ...
            #     = singlePush(dref, mu, IDctrlNode, IDctrlDOF, nSteps)
            maxDriftPiso, maxDriftTecho, maxDriftTecho2, VBasal_v, MaxDriftStory_v, DriftTecho_v, DriftStory_m, \
                PD_Beams, PD_Cols, PD_Walls, Vu_Vn_Cols, ratio_infill, VBasal_vBare = singlePush(dref, mu, IDctrlNode,
                                                                                                 IDctrlDOF, nSteps)
        ...
        if self.ui.checkBox_SDR.isChecked() == True:
            if NIF != 0 and FNB > 0:
                VBasal_vTot = VBasal_v + VBasal_vBare
                ax.plot(MaxDriftStory_v * 100, -VBasal_vTot / Wtotal, '-', c='red', linewidth=3, label=r'$SDR Total$', marker='None')
                ax.plot(MaxDriftStory_v * 100, -VBasal_v / Wtotal, '-', c='red', linewidth=2, linestyle='dotted', label=r'$SDR Infills$', marker='None')
                ax.plot(MaxDriftStory_v * 100, -VBasal_vBare / Wtotal, '-', c='red', linewidth=2, linestyle='dashdot', label=r'$SDR Bares$', marker='None')
            else:
                ax.plot(MaxDriftStory_v * 100, -VBasal_v / Wtotal, '-', c='red', linewidth=3, label=r'$SDR$',
                        marker='None')
        if self.ui.checkBox_RDR.isChecked() == True:
            if NIF != 0 and FNB > 0:
                VBasal_vTot = VBasal_v + VBasal_vBare
                ax.plot(DriftTecho_v * 100, -VBasal_vTot / Wtotal, '-', c='blue', linewidth=3, label=r'$RDR Total$', marker='None')
                ax.plot(DriftTecho_v * 100, -VBasal_v / Wtotal, '-', c='blue', linewidth=2, linestyle='dotted', label=r'$RDR Infills$', marker='None')
                ax.plot(DriftTecho_v * 100, -VBasal_vBare / Wtotal, '-', c='blue', linewidth=2, linestyle='dashdot', label=r'$RDR Bares$', marker='None')
            else:
