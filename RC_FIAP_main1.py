...
class MyForm(QDialog):
    ...
    def Design(self):
        global Loc_span, Loc_heigth, ListNodes, Elements, DataBeamDesign, DataColDesign, WDL, WLL, WDLS, Wtotal, \
            ...
            PWall2SlabD, PWall2SlabL, PColSlabD, PColSlabL, ListNodesW1, ListNodesW2, FrameDetailing, WallDetailing, FN
    ...
    def CreateNLM(self):
        ...
        def steel_mat_regu():
            ...
            FsuPosN4 = FsuTestN4
            epsuPosN4 = max(epsyPosN4 + LgageTestN4 / phl * (epsuTestN4 - epsyPosN4), 0.04)
        ...
        Krigid = 1e15
        DataColPhl = []
        for (Ele, DC) in zip(EleCol, DataColDesign):
            fc, Ec = fcC, Ele.EcEle
        ...
        if floors_num > 2 and self.ui.radioButtonFBC.isChecked():
            ...
            # T2m = 2. * pi / omegaI2
            # print('Ta1=', T1m, 'seg')
        elif floors_num == 2 or self.ui.radioButtonMVLEM.isChecked():
            nEigenI = 1  # mode 1
            nEigenI2 = 2  # mode 2
    ...
    def Pushover(self):
        ...
        def singlePush1(dref, mu, ctrlNode, dispDir, nSteps):
            ...
            while step <= nSteps and ok == 0 and loadf > 0:
                ...
                for (Ele, DC, ind) in zip(EleCol, DataColDesign, range(num_cols)):
            ...
            return maxDriftPiso, maxDriftTecho, maxDriftTecho2, VBasal_v, MaxDriftStory_v, DriftTecho_v, \
                DriftStory_m, PD_Beams, PD_Cols, PD_Walls, Vu_Vn_Cols
        ...
        if self.ui.radioButtonFast.isChecked():
            ...
            maxDriftPiso, maxDriftTecho, maxDriftTecho2, VBasal_v, MaxDriftStory_v, DriftTecho_v, DriftStory_m, \
                PD_Beams, PD_Cols, PD_Walls, Vu_Vn_Cols = singlePush1(dref, mu, IDctrlNode, IDctrlDOF, nSteps)
        ...
        if self.ui.radioButtonForced.isChecked():
            ...
            #     = singlePush(dref, mu, IDctrlNode, IDctrlDOF, nSteps)
            maxDriftPiso, maxDriftTecho, maxDriftTecho2, VBasal_v, MaxDriftStory_v, DriftTecho_v, DriftStory_m, \
                PD_Beams, PD_Cols, PD_Walls, Vu_Vn_Cols = singlePush(dref, mu, IDctrlNode, IDctrlDOF, nSteps)
        self.ui.progressBarPushover.hide()
        ...
        DataDC = []
        DataPRA = []
        for (ind, Ele) in zip(range(num_cols), EleCol):
