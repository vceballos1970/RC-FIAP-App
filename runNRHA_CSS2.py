def runNRHA_CSS(Dt, Tmax, Loc_heigth, Loc_span, ListNodesDrift, ListNodesBasal, EleCol, EleBeam, LC, DataColPhl,
    ...
    while cIndex == 0 and controlTime <= totalTime and ok == 0:
        ...
        if ok != 0:
            print('configuración por defecto no converge en tiempo: ', op.getTime())
            for j in algoritmo:
                if j < 4:
                    op.algorithm(algoritmo[j], '-initial')

                else:
                    op.algorithm(algoritmo[j])

                # el test se hace 50 veces más
                op.test('EnergyIncr', tolInit, iterInit * 50)
                ok = op.analyze(1, Dt)
                if ok == 0:
        ...
        if ok != 0:
            ...
            # print('Desplazamiento alcanzado: ', op.nodeDisp(IDctrlNode, IDctrlDOF), 'm')
            cIndex = -1
